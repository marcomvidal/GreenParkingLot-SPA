...
const checkInTime = (timeAgo: DurationLike) =>
  DateTime.now().minus(timeAgo).toISO().substr(0, 16);
...
const CHECK_INS: CheckIn[] = [
  ...
  { id: 4, spotId: SPOTS[3].id!, carId: CARS[2].id, startTime: checkInTime({ minutes: 30 }) },
  { id: 5, spotId: SPOTS[4].id!, carId: CARS[3].id, startTime: checkInTime({ hours: 2, minutes: 15 }) },
  { id: 6, spotId: SPOTS[5].id!, carId: CARS[4].id, startTime: checkInTime({ hours: 1, minutes: 15 }) },
  { id: 8, spotId: SPOTS[7].id!, carId: CARS[5].id, startTime: checkInTime({ hours: 1, minutes: 30 }) },
];
